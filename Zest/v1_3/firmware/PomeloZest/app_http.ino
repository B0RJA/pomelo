...
bool app_http_run(uint8_t buttonFlags, uint8_t serviceFlags)
{
...
void app_http_draw()
{
  if (wifi_reconnecting)
  {
    ...
    sprintf(printBuf, "%lu s...", wifi_tReconnect);
  ...
  else if (!app_http_QR)
  {
    ...
    if (wifi_connected)
    {
      ...
      {
        sprintf(printBuf, "Joined SSID: %s", wifi_ssid.c_str());
        u8g2.drawStr(0,10,printBuf);
      }
...
String app_http_processorN42(const String& var)
{
  ...
  if(var == "DURATION")
  {
    sprintf(printBuf, "%lu", serviceHistTime);
    return String(printBuf);
  }
  if(var == "LIVE")
  {
  ...
  if(var == "SPECTRUM")
  {
    *app_http_jsonData = "";
    for (i = 0; i < 1023; i++)
    {
      sprintf(printBuf, "%lu ", serviceHist[i]);
      app_http_jsonData->concat(printBuf);
    }
    sprintf(printBuf, "%lu", serviceHist[1023]);
    app_http_jsonData->concat(printBuf);
...
void app_http_startServer()
{
  ...
  // Static pages, text
  app_http_webServer.on("/uPlot.iife.min.js", HTTP_GET, [](AsyncWebServerRequest *request) { request->send(200, "text/javascript", uPlot_iife_min_js); });
  app_http_webServer.on("/uPlot.min.css", HTTP_GET, [](AsyncWebServerRequest *request) { request->send(200, "text/css", uPlot_min_css); });
  app_http_webServer.on("/browserconfig.xml", HTTP_GET, [](AsyncWebServerRequest *request) { request->send(200, "text/html", browserconfig_xml); });
  app_http_webServer.on("/site.webmanifest", HTTP_GET, [](AsyncWebServerRequest *request) { request->send(200, "text/html", site_webmanifest); });

  // Static resources, binary
  app_http_webServer.on("/android-chrome-192x192.png", HTTP_GET, [](AsyncWebServerRequest *request) { AsyncWebServerResponse *response = request->beginResponse(200, "image/png", android_chrome_192x192_png, 14447); request->send(response); });
  app_http_webServer.on("/android-chrome-512x512.png", HTTP_GET, [](AsyncWebServerRequest *request) { AsyncWebServerResponse *response = request->beginResponse(200, "image/png", android_chrome_512x512_png, 40687); request->send(response); });
  app_http_webServer.on("/apple-touch-icon.png", HTTP_GET, [](AsyncWebServerRequest *request) { AsyncWebServerResponse *response = request->beginResponse(200, "image/png", apple_touch_icon_png, 9007); request->send(response); });
  app_http_webServer.on("/favicon-16x16.png", HTTP_GET, [](AsyncWebServerRequest *request) { AsyncWebServerResponse *response = request->beginResponse(200, "image/png", favicon_16x16_png, 1106); request->send(response); });
  app_http_webServer.on("/favicon-32x32.png", HTTP_GET, [](AsyncWebServerRequest *request) { AsyncWebServerResponse *response = request->beginResponse(200, "image/png", favicon_32x32_png, 2151); request->send(response); });
  app_http_webServer.on("/favicon.ico", HTTP_GET, [](AsyncWebServerRequest *request) { AsyncWebServerResponse *response = request->beginResponse(200, "image/vnd.microsoft.icon", favicon_ico, 15086); request->send(response); });
  app_http_webServer.on("/mstile-144x144.png", HTTP_GET, [](AsyncWebServerRequest *request) { AsyncWebServerResponse *response = request->beginResponse(200, "image/png", mstile_144x144_png, 8925); request->send(response); });
  app_http_webServer.on("/mstile-150x150.png", HTTP_GET, [](AsyncWebServerRequest *request) { AsyncWebServerResponse *response = request->beginResponse(200, "image/png", mstile_150x150_png, 8717); request->send(response); });
  app_http_webServer.on("/mstile-310x150.png", HTTP_GET, [](AsyncWebServerRequest *request) { AsyncWebServerResponse *response = request->beginResponse(200, "image/png", mstile_310x150_png, 9366); request->send(response); });
  app_http_webServer.on("/mstile-310x310.png", HTTP_GET, [](AsyncWebServerRequest *request) { AsyncWebServerResponse *response = request->beginResponse(200, "image/png", mstile_310x310_png, 17934); request->send(response); });
  app_http_webServer.on("/mstile-70x70.png", HTTP_GET, [](AsyncWebServerRequest *request) { AsyncWebServerResponse *response = request->beginResponse(200, "image/png", mstile_70x70_png, 6320); request->send(response); });
  app_http_webServer.on("/safari-pinned-tab.svg", HTTP_GET, [](AsyncWebServerRequest *request) { request->send(200, "text/html", safari_pinned_tab_svg); });

  // Dynamic pages start here: index, wifi, time, spectrum
  app_http_webServer.on("/spectrum.n42", HTTP_GET, [](AsyncWebServerRequest *request) { request->send(200, "application/octet-stream", app_http_n42File, app_http_processorN42); });
  ...
  app_http_webServer.on("/wifi.htm", HTTP_ANY, [](AsyncWebServerRequest *request)
  {
    ...
    request->send(200, "text/html", wifi_htm); 
  });
  ...
  app_http_webServer.on("/time.htm", HTTP_ANY, [](AsyncWebServerRequest *request)
  {
    ...
    request->send(200, "text/html", time_htm); 
  });
  ...
  app_http_webServer.on("/", HTTP_GET, [](AsyncWebServerRequest *request) 
  {
    ...
    request->send(200, "text/html", index_htm, app_http_processorN42); 
  });
  app_http_webServer.on("/hist.json", HTTP_GET, [](AsyncWebServerRequest *request) { request->send(200, "application/json", app_http_histJson, app_http_processorHist); });
