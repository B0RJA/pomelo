...
bool app_tcp_run(uint8_t buttonFlags, uint8_t serviceFlags)
{
...
bool app_tcp_stream(uint8_t data)
{
  app_tcp_txBuf[app_tcp_txBufN] = data;
  app_tcp_txBufN++;

  if (app_tcp_txBufN == APP_TCP_BUF_LEN)
  {
    // This is the last byte we could ingest
    return false;
  }
  else
  {
